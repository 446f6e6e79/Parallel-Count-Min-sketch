    ...
    */
    ...
    int main(int argc, char **argv) {
        ...
        double io_time = 0.0, compute_time = 0.0, busy_wait_time = 0.0;
        ...
        if(!cms) {
            fprintf(stderr, "Error creating Count-Min Sketch on rank %d\n", rank);
            safe_cleanup(NULL, NULL, &fh);
            return -1;
        }
        ...
        uint8_t *buffer_curr = malloc(BUFFER_SIZE);
        if(!buffer_curr) {
            fprintf(stderr, "Error allocating memory for buffer on rank %d\n", rank);
            safe_cleanup(NULL, NULL, &fh);
            return -1;
        }
        // Allocate the buffer where the non-blocking read will write into
        uint8_t *buffer_next = malloc(BUFFER_SIZE);
        if(!buffer_next) {
            fprintf(stderr, "Error allocating memory for buffer on rank %d\n", rank);
            safe_cleanup(NULL, NULL, &fh);
            return -1;
        ...
        if (addresses_read == -1) {
            ...
            safe_cleanup(buffer_curr, cms, &fh);
            return -1;
        }
        
        // We have already read the first buffer, so we can start processing
        MPI_Offset total_read = addresses_read;
        ...
        while (total_read < local_addresses) {
            ...
            MPI_Request request;
            // Start non-blocking read for the next buffer if there are more IPs to read
            if (total_read + current_count < local_addresses) {
                double io_start = MPI_Wtime();
                non_blocking_read_buffer(fh, buffer_next, current_start, current_count, &request);
                io_time += MPI_Wtime() - io_start;
            }
        ...
        if(rank == 0){
            write_execution_info(argv[2], comm_sz, total_addresses, end_time - start_time, io_time, compute_time, busy_wait_time);
        }
